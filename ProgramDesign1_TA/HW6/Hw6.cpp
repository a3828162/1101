...
int main()
{
   ...
   for( int i = 0; i < numTestCases; i++ )
   {
      ...
      //addition(remainder, buffer, remainderDegree, bufferDegree);
...
void addition( int *addend, int *adder, int &addendDegree, int adderDegree )
{
    //if (addendDegree < adderDegree) addendDegree = adderDegree;

    addendDegree = std::max(addendDegree, adderDegree);
    for (int i = 0; i <= adderDegree; ++i) addend[i] += adder[i];
    for (int i = addendDegree; i > 0 && !addend[i]; --i,--addendDegree);
}
...
void subtraction( int *minuend, int *subtrahend, int &minuendDegree, int subtrahendDegree )
{
   ...
   for (int i = minuendDegree; i > 0 && !minuend[i]; --i, --minuendDegree);
                     ...
                     int multiplicandDegree, int multiplierDegree, int productDegree )
{
    ...
    for (int i = productDegree; i > 0 && !product[i]; --i, --productDegree);
}
...
void division( int *dividend, int *divisor, int *quotient, int *remainder,
               int dividendDegree, int divisorDegree, int quotientDegree, int &remainderDegree )
{
    std::copy(dividend, dividend + dividendDegree + 1, remainder);

    int degree = quotientDegree;
    while (remainderDegree > divisorDegree && degree>=0 )
    {
        quotient[degree] = remainder[remainderDegree] / divisor[divisorDegree];
        int monomialDegree = degree;
        int* monomial = new int[degree + 1]();
        monomial[degree] = quotient[degree];

        int bufferDegree = degree + divisorDegree;
        int* buffer = new int[bufferDegree + 1]();
