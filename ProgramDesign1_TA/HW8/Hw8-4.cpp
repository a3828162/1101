...
void addition( vector< Term > &addend, const vector< Term > adder )
{
   ...
   while (i < addend.size() || j < adder.size())
   {
       if (i == addend.size())
       {
           while (j < adder.size()) sum[k].expon = adder[j].expon, sum[k++].coef = adder[j++].coef;
           break;
       }
       else if (j == adder.size())
       {
           while (i < addend.size()) sum[k].expon = addend[i].expon, sum[k++].coef = addend[i++].coef;
           break;
       }

       if (addend[i].expon > adder[j].expon) sum[k].expon = addend[i].expon, sum[k++].coef = addend[i++].coef;
       else if (addend[i].expon < adder[j].expon) sum[k].expon = adder[j].expon, sum[k++].coef = adder[j++].coef;
       else
       {
           if (addend[i].coef + adder[j].coef != 0) sum[k].expon = addend[i].expon, sum[k++].coef = addend[i].coef + adder[j].coef;
           i++, j++;
       }
   }
...
void multiplication( const vector< Term > &multiplicand, const vector< Term > &multiplier, vector< Term > &product )
{
   ...
   if( multiplicand.size() != 0 && multiplier.size() != 0 )
   {
       ...
       {
           for (size_t j = 0; j < multiplicand.size(); ++j) buffer[j].expon = multiplicand[j].expon + multiplier[i].expon, buffer[j].coef = multiplicand[j].coef * multiplier[i].coef;
               ...
               vector< Term > &quotient, vector< Term > &remainder )
{
   ...
   {
       temp[quotient.size()].coef = remainder[0].coef / divisor[0].coef, temp[quotient.size()].expon = remainder[0].expon - divisor[0].expon;
       monomial[0].expon = temp[quotient.size()].expon, monomial[0].coef = temp[quotient.size()].coef;
