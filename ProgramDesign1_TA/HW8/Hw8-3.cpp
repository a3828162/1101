...
void Polynomial::addition( Polynomial adder )
{
   ...
   while (i < size || j < adder.size)
   {
       ...
       {
           while (i < size) sum.terms[k].expon = terms[i].expon, sum.terms[k++].coef = terms[i++].coef;
           break;
       }

       if (terms[i].expon > adder.terms[j].expon) sum.terms[k].expon = terms[i].expon, sum.terms[k++].coef = terms[i++].coef;
       else if (terms[i].expon < adder.terms[j].expon) sum.terms[k].expon = adder.terms[j].expon, sum.terms[k++].coef = adder.terms[j++].coef;
       else
       {
           if (terms[i].coef + adder.terms[j].coef != 0) sum.terms[k].expon = terms[i].expon, sum.terms[k++].coef = terms[i].coef + adder.terms[j].coef;
           i++, j++;
       }
   }
...
void Polynomial::multiplication( Polynomial &multiplier, Polynomial &product )
{
   ...
   if( size != 0 && multiplier.size != 0 )
   {
       ...
       {
           for (int j = 0; j < size; ++j) buffer.terms[j].expon = terms[j].expon + multiplier.terms[i].expon, buffer.terms[j].coef = terms[j].coef * multiplier.terms[i].coef;
...
void Polynomial::division( Polynomial &divisor, Polynomial &quotient, Polynomial &remainder )
{
   ...
   {
       temp.terms[quotient.size].coef = remainder.terms[0].coef / divisor.terms[0].coef, temp.terms[quotient.size].expon = remainder.terms[0].expon - divisor.terms[0].expon;
       monomial.terms[0].expon = temp.terms[quotient.size].expon, monomial.terms[0].coef = temp.terms[quotient.size].coef;
