...
void addition( Term *&addend, int &addendSize, Term *adder, int adderSize )
{
   ...
   while (i < addendSize || j < adderSize)
   {
       if (i == addendSize)
       {
           while (j < adderSize) sum[k].expon = adder[j].expon, sum[k++].coef = adder[j++].coef;
           break;
       }
       else if (j == adderSize)
       {
           while (i < addendSize) sum[k].expon = addend[i].expon, sum[k++].coef = addend[i++].coef;
           break;
       }
   ...
   Term *multiplier, int multiplierSize, Term *&product, int &productSize )
{
   ...
   if( multiplicandSize != 0 && multiplierSize != 0 )
   {
       ...
       {
           for (int j = 0; j < multiplicandSize; ++j) buffer[j].expon = multiplicand[j].expon + multiplier[i].expon, buffer[j].coef = multiplicand[j].coef * multiplier[i].coef;
               ...
               Term *&quotient, int &quotientSize, Term *&remainder, int &remainderSize )
{
   ...
   {
       temp[quotientSize].coef = remainder[0].coef / divisor[0].coef, temp[quotientSize].expon = remainder[0].expon - divisor[0].expon;
