...
void addition( Polynomial &addend, const Polynomial adder )
{
   ...
   while (i < addend.size || j < adder.size)
   {
       if (i == addend.size)
       {
           while (j < adder.size) sum.terms[k].expon = adder.terms[j].expon, sum.terms[k++].coef = adder.terms[j++].coef;
           break;
       }
       else if (j == adder.size)
       {
           while (i < addend.size) sum.terms[k].expon = addend.terms[i].expon, sum.terms[k++].coef = addend.terms[i++].coef;
           break;
       }

       if (addend.terms[i].expon > adder.terms[j].expon) sum.terms[k].expon = addend.terms[i].expon, sum.terms[k++].coef = addend.terms[i++].coef;
       else if (addend.terms[i].expon < adder.terms[j].expon) sum.terms[k].expon = adder.terms[j].expon, sum.terms[k++].coef = adder.terms[j++].coef;
       else
       {
           if (addend.terms[i].coef + adder.terms[j].coef != 0) sum.terms[k].expon = addend.terms[i].expon, sum.terms[k++].coef = addend.terms[i].coef + adder.terms[j].coef;
           i++, j++;
       }
   }
...
void multiplication( const Polynomial &multiplicand, const Polynomial &multiplier, Polynomial &product )
{
   ...
   if( multiplicand.size != 0 && multiplier.size != 0 )
   {
       ...
       {
           for (int j = 0; j < multiplicand.size; ++j) buffer.terms[j].expon = multiplicand.terms[j].expon + multiplier.terms[i].expon, buffer.terms[j].coef = multiplicand.terms[j].coef * multiplier.terms[i].coef;
               ...
               Polynomial &quotient, Polynomial &remainder )
{
   ...
   {
       temp.terms[temp.size].coef = remainder.terms[0].coef / divisor.terms[0].coef, temp.terms[temp.size].expon = remainder.terms[0].expon - divisor.terms[0].expon;
       monomial.terms[0].expon = temp.terms[temp.size].expon, monomial.terms[0].coef = temp.terms[temp.size].coef;
